...
def llama3(sentences, model=config.MODEL, max_retries=5, delay=2.0, save_prompt=False):
    prompt_path = os.path.join(config.TOPIC_PROMPT_PATH,config.PROMPT_VERSION.split("_")[1] + ".txt")
    print("Prompt path:", config.PROMPT_VERSION)
...
def summarize_topics():
    ...
    for file in os.listdir(config.SEGMENTATION_OUTPUT_PATH):
        ...
        output_file = os.path.join(
            config.SUMMARIZATION_OUTPUT_PATH, file.replace(".json", ".json")
        )
        ...
        with open(os.path.join(config.SEGMENTATION_OUTPUT_PATH, file), "r") as f:
            topics = json.load(f)
            print("Raw topics loaded:", topics)
        ...
        if config.PROMPT_VERSION.split("_")[1] in ["e-t-", "e-t+"]:
            ...
            transcript_emotion_file = os.path.join(
                config.PROCESSED_TRANSCRIPT_PATH, file.replace(".txt", ".json")
            )
        else:
            transcript_emotion_file = os.path.join(
        ...
        with open(transcript_emotion_file, "r") as f:
            ...
            print(transcript_json_emotion)

        # build per-topic transcripts
        topics[0]["start_sentence"] = 0
        for topic in topics:
