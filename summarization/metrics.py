        ...
        if isinstance(gt, list) and isinstance(pred, list):
            for gt_item, pred_item in zip(gt, pred):
                for speaker in gt_item:
                    gt_seq = [
                        s.lower() if isinstance(s, str) else random.choice(s).lower()
                        for s in gt_item[speaker]
                    ]
                    pred_seq = [
                        s.lower() for s in process_list(pred_item.get(speaker, []))
                    ]
                    pred_seq = [s if s in config.EMOTION_LIST else "other" for s in pred_seq]
                    gt_seq = normalize_emotion(gt_seq)
                    pred_seq = normalize_emotion(pred_seq)
            ...
            for speaker in gt:
                ...
                pred_seq = [
                    s.lower() for s in process_list(pred.get(speaker, []))
                ]
                pred_seq = [s if s in config.EMOTION_LIST else "other" for s in pred_seq]
                gt_seq = normalize_emotion(gt_seq)
                pred_seq = normalize_emotion(pred_seq)
...
def main():
    ...
    for mode in modes:
        ...
        result_path = os.path.join(
            ...
            f"{config.PROMPT_VERSION}_{suffix}.json"
        )
        result_path = os.path.join(
            config.EMOTION_EVAL_RESULT_PATH,"amitopic3b_e+t+_topic.json")
