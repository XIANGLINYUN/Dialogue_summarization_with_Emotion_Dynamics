...
def evaluate_overall_summaries():
    ...
    out_path = os.path.join(config.EMOTION_EVAL_RESULT_PATH, config.SETUP_SET[0] +"_overall_individual.json")
    if os.path.exists(out_path):
        print(f"⏩ Skipping overall evaluation, already exists at {out_path}")
        return
    emotion_labels_gt_pred = {}

    for file in os.listdir(config.OVERALL_SUMMARIZATION_OUTPUT_PATH_WITH_INDIVIDUAL):
