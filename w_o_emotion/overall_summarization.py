...
def llama3_overall(sub_topic_summaries, model=config.MODEL, save_prompt=True):
    prompt_path = os.path.join(config.OVERALL_PROMPT_PATH,config.OVERALL_PROMPT_VERSION.split("_")[1] + ".txt")
    ...
    with open(prompt_path, "r", encoding="utf-8") as f:
        ...
        user_message = user_part.strip()+ json.dumps(sub_topic_summaries)
...
def summarize_overall():
    ...
    for file in os.listdir(config.SUMMARIZATION_OUTPUT_PATH):
        if not file.endswith(".json"):
            continue
        if os.path.exists(os.path.join(config.OVERALL_SUMMARIZATION_OUTPUT_PATH, file.replace(".json", ".txt"))):
        ...
        topic_summaries = {topic: content["summary"].split(":", 1)[1].strip() if ":" in content["summary"] else content["summary"] for topic, content in topic_transcript_summary.items()}
