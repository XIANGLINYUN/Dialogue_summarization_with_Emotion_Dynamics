...
def run_topic_blanc_eval(mode="help"):
    ...
    """
    stage1_summary_path = os.path.join(config.INDIVIDUAL_SUMMARIZATION_OUTPUT_PATH, config.SETUP_SET[0])
    ...
    for file in os.listdir(stage1_summary_path):
        ...
        with open(summary_file, "r") as f:
            data = json.load(f)
            transcript = ""
            for key, value in data["transcript"].items():
                transcript = transcript + " " + value.get("utterance", "").strip()
            summary = data["summary"].strip()

            try:
                score = blanc_model.eval_once(transcript, summary)
            except Exception as e:
                print(f"⚠️ BLANC failed for {file} - {key}: {e}")
                continue

            overall_score += float(score)
            print(f"Processed {file} - {key}: BLANC-{mode.capitalize()} score = {score:.4f}")
            count += 1
...
def run_overall_blanc_eval(mode="help"):
    over_summary_path = os.path.join(
